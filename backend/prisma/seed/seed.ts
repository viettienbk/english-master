...
async function main() {
  ...
  for (const extraTopic of allExtraData) {
    const existingTopic = mergedVocabulary.find(
      (t: any) => t.name === extraTopic.name,
    );
    if (existingTopic) {
      // Avoid duplicate words
      const existingWordTexts = new Set(
        existingTopic.words.map((w: any) => w.word.toLowerCase()),
      );
      const newWords = extraTopic.words.filter(
        (w: any) => !existingWordTexts.has(w.word.toLowerCase()),
      );
  ...
  // Seed vocabulary topics and words
  ...
  const listeningLessons = [
    {
      ...
      transcript:
        'Good morning. Welcome to the check-in counter. May I see your passport and boarding pass please? Here you go. Thank you. Are you checking any luggage today? Yes, I have one suitcase. Please place it on the scale. Your flight to London departs from gate B12 at 3 PM. Have a nice flight!',
      ...
      translation:
        'Chào buổi sáng. Chào mừng đến quầy check-in. Tôi có thể xem hộ chiếu và thẻ lên máy bay của bạn không? Đây ạ. Cảm ơn. Bạn có gửi hành lý nào hôm nay không? Vâng, tôi có một vali. Vui lòng đặt lên cân. Chuyến bay của bạn đến London khởi hành từ cửa B12 lúc 3 giờ chiều. Chúc bạn chuyến bay vui vẻ!',
      order: 1,
    },
    {
      ...
      translation:
        'Chào buổi tối. Chào mừng đến nhà hàng The Garden. Cho hai người một bàn. Mời đi lối này. Đây là thực đơn. Tôi có thể lấy cho bạn gì để uống? Tôi sẽ lấy một ly nước và cô ấy sẽ lấy nước cam. Bạn sẵn sàng gọi món chưa? Vâng, tôi muốn gà nướng và cô ấy sẽ lấy mì ý. Lựa chọn tuyệt vời!',
      order: 2,
    },
    {
      title: 'At the Doctor',
      ...
      translation:
        'Xin chào bác sĩ. Có vấn đề gì vậy? Gần đây tôi cảm thấy rất mệt và tôi bị đau đầu không dứt. Bạn bị những triệu chứng này bao lâu rồi? Khoảng một tuần nay. Để tôi kiểm tra nhiệt độ và huyết áp. Nhiệt độ của bạn hơi cao. Tôi sẽ kê đơn thuốc cho bạn. Uống hai lần một ngày sau bữa ăn.',
      order: 3,
    },
    ...moreListeningData,
